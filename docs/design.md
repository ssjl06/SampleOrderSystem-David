...
| name | string | 시료명 |
| avgProductionTime | double | 평균 생산시간 (초/ea) |
...
| totalTime | double | 총생산시간 = avgProductionTime × actualProduction (초) |
| enqueuedAt | long long | 큐 등록 Unix epoch 초 — 재시작 후에도 경과 시간 계산 가능 |
...
예시: 주문 200ea, 재고 30ea, 수율 0.92, 평균생산시간 0.8sec/ea
```
부족분     = 200 - 30 = 170
실생산량   = ceil(170 / (0.92 × 0.9)) = ceil(170 / 0.828) = ceil(205.3) = 206
총생산시간 = 0.8 × 206 = 164.8 sec
```
...
- 전략: **FIFO** (enqueuedAt epoch 기준 정렬)
- 하나의 생산라인, 한 번에 하나의 작업 처리
- 자동 완료: `now - enqueuedAt >= totalTime(초)` 조건 충족 시 PRODUCING → CONFIRMED 자동 전환
- `ProductionService::checkAndAutoComplete()` — MainController 루프마다 호출
- 프로세스 재시작 후에도 epoch 기반으로 경과 시간 재계산하여 자동 전환
