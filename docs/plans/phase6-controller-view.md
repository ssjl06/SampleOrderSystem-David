...
**Steps:**
- [ ] DummyDataGenerator 실행 → data/ 주입 (시료 12종, 주문 36건, 생산대기 3건)
- [ ] 메인 시스템 실행 → 메인 메뉴 현황 확인 (시료 12종, 총 주문 36건, 생산대기 3건)
- [ ] [1] 시료 관리 → 목록 조회 → 12종 확인
- [ ] [4] 모니터링 → RESERVED:3 / CONFIRMED:3 / PRODUCING:3 / RELEASE:26 / REJECTED:1 확인
- [ ] [2] 주문 접수 → 시료 목록에서 임의 sampleId 선택 후 주문 생성 → RESERVED 확인
- [ ] [3] 주문 승인 → RESERVED 주문 중 재고 충분한 건 선택 → CONFIRMED 전환 확인
- [ ] [3] 주문 승인 → RESERVED 주문 중 재고 부족한 건 선택 → PRODUCING 전환 + 생산잡 추가 확인
- [ ] [5] 생산라인 → 대기 큐 FIFO 순서 확인 + 완료 처리 → 재고 증가 + CONFIRMED 전환 확인
- [ ] [6] 출고 처리 → CONFIRMED 목록에서 선택 → RELEASE 전환 확인
